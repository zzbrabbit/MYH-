...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  /* ---------- Category Quick Nav (under home banner, 200x80 tiles) ---------- */
  const catQuickRow = document.getElementById('catQuickNavRow');
  if (catQuickRow && typeof CATEGORY_LIBRARY !== 'undefined' && typeof ICON_LIBRARY !== 'undefined') {
    const QUICK_CATS = (typeof CATNAV_CATS !== 'undefined' && CATNAV_CATS.length)
      ? CATNAV_CATS
      : ['rf', 'cavitation', 'led', 'hair', 'body', 'micro', 'ultra', 'sshape'];
    // Load optional custom tile images from the admin (CatNav module)
    let catNavMap = {};
    if (typeof CatNav !== 'undefined') {
      try { await CatNav.init(); } catch (e) { /* ignore */ }
      CatNav.getAll().forEach(t => { catNavMap[t.cat] = t; });
    }
    catQuickRow.innerHTML = QUICK_CATS.map((cat) => {
      const meta = CATEGORY_LIBRARY[cat];
      if (!meta) return '';
      const custom = catNavMap[cat];
      const svg = ICON_LIBRARY[meta.icon] || ICON_LIBRARY.rf || '';
      const hasCustom = !!(custom && custom.image);
      const iconHtml = hasCustom
        ? `<img class="cat-tile-img" src="${custom.image}" alt="${custom.alt || meta.label}" onerror="this.style.display='none';this.nextElementSibling.style.display='';">`
        : '';
      const fallbackStyle = hasCustom ? ' style="display:none;"' : '';
      return `<button class="cat-tile" data-cat="${cat}" aria-label="${meta.label}">
        ${iconHtml}<span class="cat-tile-icon"${fallbackStyle}>${svg}</span>
        <span class="cat-tile-label">${meta.label}</span>
      </button>`;
    }).join('');

    catQuickRow.addEventListener('click', (ev) => {
      const tile = ev.target.closest('.cat-tile');
      if (!tile) return;
      // Highlight the clicked tile
      catQuickRow.querySelectorAll('.cat-tile.active').forEach((el) => el.classList.remove('active'));
      tile.classList.add('active');
      // Navigate to the category after a short pause so the highlight is visible
      const cat = tile.dataset.cat;
      setTimeout(() => { window.location.href = 'products.html?cat=' + encodeURIComponent(cat); }, 280);
    });
  }
  ...
  if (bannerSection) {
    ...
    if (!banners.length) {
      ...
      const fallbackGrad = (i) => grads.length ? grads[i % grads.length].value : 'linear-gradient(135deg,#4E9EF6,#1877F2)';
  ...
  function openProductDetail(productId) {
    ...
    content.innerHTML = `
      ...
      <div class="detail-lower">${specsHtml}<div class="detail-desc-block"><h4 class="detail-desc-title">Description</h4><div class="detail-desc detail-desc-lower">${product.description}</div></div></div>
      ${relatedHtml}`;
  ...
  // Bind click on product cards & blog articles (event delegation)
  document.addEventListener('click', (e) => {
    ...
    if (article) {
      e.preventDefault();
      openBlogArticle(article.dataset.articleId);
    }
  });

  // Keyboard activation (Enter / Space) for blog article cards
  document.addEventListener('keydown', (e) => {
    if (e.key !== 'Enter' && e.key !== ' ') return;
    const target = e.target.closest && e.target.closest('[data-article-id]');
    if (target) {
      e.preventDefault();
      openBlogArticle(target.dataset.articleId);
    }
  });
  ...
  if (blogGrid && (typeof Posts !== 'undefined' || typeof BLOG_ARTICLES !== 'undefined')) {
    const getBlogPosts = () => (typeof Posts !== 'undefined') ? Posts.getAll()
      : (typeof BLOG_ARTICLES !== 'undefined' ? BLOG_ARTICLES : []);
    ...
    if (blogFeatured && featuredArticle) {
      blogFeatured.innerHTML = `
        <div class="blog-featured-card reveal" data-article-id="${featuredArticle.id}" role="button" tabindex="0" aria-label="Read: ${featuredArticle.title}">
      ...
      const featuredEl = blogFeatured.querySelector('.blog-featured-card');
      if (featuredEl) revealObserver.observe(featuredEl);
    }
    ...
    function renderBlogGrid(category) {
      const list = category === 'all'
        ? blogPosts.filter(a => a.id !== (featuredArticle && featuredArticle.id))
        : blogPosts.filter(a => a.category === category && a.id !== (featuredArticle && featuredArticle.id));
      if (list.length === 0) {
      ...
      blogGrid.innerHTML = list.map((a, i) => `
        <article class="blog-card reveal reveal-delay-${(i % 4) + 1}" data-article-id="${a.id}" role="button" tabindex="0" aria-label="Read: ${a.title}">
    ...
    if (blogSidebarCats) {
      const counts = {};
      blogPosts.forEach(a => { counts[a.category] = (counts[a.category] || 0) + 1; });
      blogSidebarCats.innerHTML = `<li><button class="sidebar-cat active" data-category="all"><span>All Articles</span><span class="cat-count">${blogPosts.length}</span></button></li>` +
        BLOG_CATEGORIES.map(c => `<li><button class="sidebar-cat" data-category="${c}"><span>${c}</span><span class="cat-count">${counts[c] || 0}</span></button></li>`).join('');
      blogSidebarCats.querySelectorAll('.sidebar-cat').forEach(btn => {
        btn.addEventListener('click', () => {
          blogSidebarCats.querySelectorAll('.sidebar-cat').forEach(b => b.classList.remove('active'));
          btn.classList.add('active');
          document.querySelectorAll('.blog-filter-btn').forEach(b => b.classList.toggle('active', b.dataset.category === btn.dataset.category));
          renderBlogGrid(btn.dataset.category);
        });
      });
    }
    ...
    if (blogPopular) {
      blogPopular.innerHTML = blogPosts.filter(a => a.id !== (featuredArticle && featuredArticle.id)).slice(0, 4).map(a => `
        <li class="blog-popular-item" data-article-id="${a.id}" role="button" tabindex="0" aria-label="Read: ${a.title}">
    ...
    if (blogTagsEl) {
      const staticTags = ['RF', 'Cavitation', 'LED', 'IPL', 'Microneedling', 'EMS', 'Cryo', 'Dermabrasion', 'Anti-Aging', 'Slimming', 'Acne', 'Skin Care'];
      const postTags = [];
      blogPosts.forEach(p => (p.tags || []).forEach(t => { if (t && postTags.indexOf(t) === -1) postTags.push(t); }));
      const tags = Array.from(new Set(staticTags.concat(postTags))).slice(0, 18);
  ...
  function ensureBlogModal() {
    ...
    blogModal.className = 'blog-overlay';
    blogModal.innerHTML = `
      <div class="blog-modal">
        <button class="blog-close" aria-label="Close">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
        </button>
        <article class="blog-article">
          <div class="blog-article-hero" id="blogArticleHero"><!-- image rendered by JS --></div>
          <div class="blog-article-inner">
            <div id="blogArticleBody"><!-- body rendered by JS --></div>
            <div class="blog-article-foot">
              <a href="blog.html" class="btn btn-outline btn-sm">Back to Blog</a>
              <a href="contact.html" class="btn btn-primary btn-sm btn-arrow">Ask an Expert</a>
            </div>
          </div>
        </article>
      </div>`;
    document.body.appendChild(blogModal);
    blogModal.querySelector('.blog-close').addEventListener('click', closeBlogModal);
    blogModal.addEventListener('click', (e) => { if (e.target === blogModal) closeBlogModal(); });
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && blogModal.classList.contains('open')) closeBlogModal();
    });
    return blogModal;
  }
  ...
  function openBlogArticle(articleId) {
    if (!articleId) return;
    const post = (typeof Posts !== 'undefined') ? Posts.getPost(articleId) : null;
    const fallback = (typeof BLOG_ARTICLES !== 'undefined') ? BLOG_ARTICLES.find(a => a.id === articleId) : null;
    const article = post || fallback;
    if (!article) return;

    ensureBlogModal();
    const heroEl = blogModal.querySelector('#blogArticleHero');
    const bodyEl = blogModal.querySelector('#blogArticleBody');
    const contentHtml = article.content && article.content.trim()
      ? article.content
      : (article.excerpt ? '<p>' + article.excerpt + '</p>' : '');

    heroEl.innerHTML = article.image
      ? `<img src="${article.image}" alt="${article.title}" onerror="this.parentElement.style.display='none';">`
      : '';

    const tagsHtml = (article.tags || []).length
      ? `<div class="blog-article-tags">${article.tags.map(t => `<span class="blog-tag">${t}</span>`).join('')}</div>`
      : '';

    bodyEl.innerHTML = `
      <span class="blog-cat-badge">${article.category}</span>
      <h1>${article.title}</h1>
      <div class="blog-meta blog-meta-lg">
        <span>${article.author || 'MYHBeauty Team'}</span><span>&middot;</span>
        <span>${article.date}</span><span>&middot;</span><span>${article.readTime}</span>
      </div>
    ...
    if (blogModal.querySelector('.blog-modal')) blogModal.querySelector('.blog-modal').scrollTop = 0;
  }
  ...
  if (homeBlogPreview && (typeof Posts !== 'undefined' || typeof BLOG_ARTICLES !== 'undefined')) {
    const previewPosts = (typeof Posts !== 'undefined') ? Posts.getAll()
      : (typeof BLOG_ARTICLES !== 'undefined' ? BLOG_ARTICLES : []);
    homeBlogPreview.innerHTML = previewPosts.slice(0, 3).map((a, i) => `
      <article class="blog-card reveal reveal-delay-${(i % 4) + 1}" data-article-id="${a.id}" role="button" tabindex="0" aria-label="Read: ${a.title}">
        <div class="blog-card-img"><img src="${a.image}" alt="${a.title}" onerror="this.style.display='none';"></div>
        <div class="blog-card-body">
          <span class="blog-cat-badge">${a.category}</span>
